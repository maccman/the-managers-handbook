...
Clarify what type of meeting you are having when it is created. Since we have already tackled how to run a one-on-one meeting (see [Running one-on-ones](/coaching-and-feedback/running-one-on-ones), here we are going to focus on staff meetings and decision meetings. These are the two that tend to run amok anyhow.
...
Any work to be done must be recorded in Asana; otherwise, it'll just be lost. We use [Impeccable agreements](/working-as-a-team/impeccable-agreements) for this. Every task must have an owner, a due date, and a good description.
